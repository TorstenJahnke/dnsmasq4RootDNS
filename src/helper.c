...
int create_helper(int event_fd, int err_fd, uid_t uid, gid_t gid, long max_fd)
{
  ...
  while(1)
      ...
      for (i = 0; (i < data.hwaddr_len) && (i < DHCP_CHADDR_MAX); i++)
	{
      ...
      if (data.hostname_len < 0 || data.ed_len < 0 || data.clid_len < 0 ||
	  data.hostname_len > MAXDNAME || data.ed_len > MAXDNAME || data.clid_len > MAXDNAME)
	continue;
...
static int buff_alloc(size_t size)
{
  if (size > buf_size)
    {
...
void queue_script(int action, struct dhcp_lease *lease, char *hostname, time_t now)
{
  ...
  if (!buff_alloc(sizeof(struct script_data) +  clid_len + ed_len + hostname_len))
    return;

...
void init_bpf(void)
{
  ...
  die(_("cannot create DHCP BPF socket: all /dev/bpf devices busy"), NULL, EC_BADNET);
}
		  ...
		  struct in_addr iface_addr, struct ifreq *ifr)
{
  ...
  ip.ip_sum = 0;
  {
    u16 ip_words[sizeof(struct ip) / 2];
    memcpy(ip_words, &ip, sizeof(ip_words));
  ...
  sum += (ip.ip_dst.s_addr >> 16) & 0xffff;
  {
    ...
    for (i = 0; i < sizeof(struct udphdr)/2; i++)
      {
	memcpy(&val, (unsigned char *)&udp + i * 2, sizeof(val));
	sum += val;
      }
    for (i = 0; i < (len + 1) / 2; i++)
      {
	memcpy(&val, (unsigned char *)mess + i * 2, sizeof(val));
	sum += val;
      }
  }
  while (sum>>16)
  ...
  if (ioctl(daemon->dhcp_raw_fd, BIOCSETIF, ifr) == -1)
    return;

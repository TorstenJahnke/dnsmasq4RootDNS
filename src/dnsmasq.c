...
#  endif
  ...
  debug_mode = option_bool(OPT_DEBUG) ? 1 : 0;
...
#  endif
  ...
  if (daemon->port == 0)
      ...
      if (daemon->cachesize != 0)
	{
	  ...
	  if (daemon->cachesize > 10000 && !option_bool(OPT_RECURSIVE))
  ...
  if (daemon->port != 0 && option_bool(OPT_RECURSIVE))
    {
      if (daemon->root_hints_file)
	{
	  if (!recursive_load_root_hints(daemon->root_hints_file))
	    {
	      my_syslog(LOG_WARNING, _("falling back to built-in root server hints"));
	      recursive_init_root_servers();
	    }
	}
      else
	recursive_init_root_servers();
      ...
      if (daemon->root_zone_file)
	{
	  int loaded = recursive_load_root_zone(daemon->root_zone_file);
	  if (loaded > 0)
	    my_syslog(LOG_INFO, _("iterative resolution enabled (--forward-rootDNS): "
				  "%d TLD delegations pre-loaded from root zone"), loaded);
	  else
	    my_syslog(LOG_WARNING, _("failed to load root zone from %s, "
				     "falling back to dynamic resolution"), daemon->root_zone_file);
	}
      else
	my_syslog(LOG_INFO, _("iterative resolution enabled (--forward-rootDNS): resolving from root servers"));

      if (!option_bool(OPT_ROOT_PRIMING))
	set_option_bool(OPT_ROOT_PRIMING);

      if (option_bool(OPT_ROOT_PRIMING))
	recursive_prime_root();
      else
	my_syslog(LOG_INFO, _("recursive: root priming disabled"));

      if (daemon->iterative_async_limit == -1)
	daemon->iterative_async_limit = 2;

      if (daemon->iterative_async_limit > 0)
	my_syslog(LOG_INFO, _("recursive: async NS resolution enabled (max %d concurrent)"),
		  daemon->iterative_async_limit);
    }
  ...
  if (daemon->port != 0 && option_bool(OPT_NO_RESOLV))
	...
	my_syslog(LOG_WARNING, _("warning: ignoring resolv-file flag because no-resolv is set"));
      ...
      if (!daemon->servers && !option_bool(OPT_RECURSIVE))
	{
#ifdef HAVE_DBUS
	  if (option_bool(OPT_DBUS))
...
#  ifdef HAVE_LINUX_NETWORK
  ...
  if (netlink_warn)
    my_syslog(LOG_WARNING, "%s", netlink_warn);
#  endif
...
void send_event(int fd, int event, int data, char *msg)
{
  ...
  if (fd == -1)
    ...
    {
      ssize_t rc;
      while ((rc = writev(fd, iov, msg ? 2 : 1)) == -1 && errno == EINTR);
      (void)rc;
    }
}
...
#define MAX_EVENT_MSG 4096
  if (evp->msg_sz > 0 && evp->msg_sz <= MAX_EVENT_MSG)
    {
      if ((buf = malloc(evp->msg_sz + 1)) &&
      ...
      else
	{
	  ...
	  int remaining = evp->msg_sz;
	  if (buf) free(buf);
	  while (remaining > 0)
	    {
	      int chunk = remaining < (int)sizeof(drain) ? remaining : (int)sizeof(drain);
	      if (!read_write(fd, drain, chunk, RW_READ))
		break;
	      remaining -= chunk;
	    }
	}
...
int icmp_ping(struct in_addr addr)
{
  ...
  for (j = 0, i = 0; i < sizeof(struct icmp) / 2; i++)
    {
      u16 val;
      memcpy(&val, (unsigned char *)&packet.icmp + i * 2, sizeof(val));
      j += val;
    }
  while (j>>16)
    j = (j & 0xffff) + (j >> 16);  

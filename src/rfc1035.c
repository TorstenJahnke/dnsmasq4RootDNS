		 ...
		 char *name, int func, unsigned int parm)
{
  ...
  const unsigned char *bigmap_src = (const unsigned char *)name;
		...
		if ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z'))
		  {
		    ...
		    if (bigmap_posn < bigmap_size && bigmap_counter-- == 0)
		      {
			memcpy(&bitmap, bigmap_src + bigmap_posn * sizeof(unsigned int), sizeof(unsigned int));
			bigmap_posn++;
...
int private_net6(struct in6_addr *a, int ban_localhost)
{
  ...
  if (IN6_IS_ADDR_V4MAPPED(a))
    {
      struct in_addr v4;
      memcpy(&v4.s_addr, (const char *)a + 12, sizeof(v4.s_addr));
  ...
  return
    ...
    ({u32 a0; memcpy(&a0, a, sizeof(a0)); a0;}) == htonl(0x20010db8); /* RFC 6303 4.6 */
}
		  ...
		  if (*rrdesc == -1 && ardlen <= (int)RR_IMDATALEN)
		    {
		       ...
		       addr.rrdata.datalen = (unsigned char)ardlen;
...
#define CHECK_LIMIT(size) \
  do { if (limit && p + (size) > (unsigned char*)limit) goto truncated; } while(0)

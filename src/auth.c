...
static struct addrlist *find_addrlist(struct addrlist *list, int flag, union all_addr *addr_u)
{
  do {
	...
	netmask.s_addr = (list->prefixlen == 0) ? 0 : htonl(~(in_addr_t)0 << (32 - list->prefixlen));
  ...
  if (OPCODE(header) != QUERY)
      ...
      if (qtype == T_PTR && flag)
	{
	  ...
	  if ((crecp = cache_find_by_addr(NULL, &addr, now, flag)))
	    do { 
	      safe_strncpy(name, cache_get_name(crecp), MAXDNAME);
		  ...
		  if (zone)
		    {
		      ...
		      snprintf(name + len, MAXDNAME - len, ".%s", zone->domain);
		    }
	  ...
	  if (candidate)
	    {
	      ...
	      safe_strncpy(name, candidate->target, MAXDNAME);
	      if (!strchr(name, '.'))
		{
		  size_t len = strlen(name);
		  snprintf(name + len, MAXDNAME - len, ".%s", zone->domain);
      ...
      if (axfr)
	{
	  ...
	  for (a = daemon->cnames; a; a = a->next)
	    if (in_zone(zone, a->alias, &cut))
	      {
		safe_strncpy(name, a->target, MAXDNAME);
		if (!strchr(name, '.'))
		  {
		    size_t len = strlen(name);
		    snprintf(name + len, MAXDNAME - len, ".%s", zone->domain);

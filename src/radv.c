...
static void send_ra_alias(time_t now, int iface, char *iface_name, struct in6_addr *dest, int send_iface)
{
  ...
  for (up = &daemon->dhcp6, context = daemon->dhcp6; context; context = tmp)
	...
	{
	  unsigned int old = (now >= context->address_lost_time) ?
	    (unsigned int)(now - context->address_lost_time) : 0;
  ...
  if (dest)
  ...
  else
    {
      ...
      if (setsockopt(daemon->icmp6fd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &send_iface, sizeof(send_iface)) == -1)
	return;
    }

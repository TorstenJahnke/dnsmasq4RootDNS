...
static int dhcp6_maybe_relay(struct state *state, unsigned char *inbuff, size_t sz,
			     struct in6_addr *client_addr, int is_unicast, time_t now, int depth);
			   ...
			   size_t sz, struct in6_addr *client_addr, time_t now)
{
  ...
  if (dhcp6_maybe_relay(&state, daemon->dhcp_packet.iov_base, sz, client_addr,
			IN6_IS_ADDR_MULTICAST(client_addr), now, 0))
	  ...
	  if (opt6_type(opt) == OPTION6_RELAY_MSG)
		...
		state->link_address = &align;
		 ...
		 relayed packet, not the original sent by the client */
	      if (!dhcp6_maybe_relay(state, opt6_ptr(opt, 0), opt6_len(opt), client_addr, 0, now, depth + 1))
		return 0;
	    }
	  else
  ...
  for (vendor = daemon->dhcp_vendors; vendor; vendor = vendor->next)
      ...
      if ((opt = opt6_find(state->packet_options, state->end, mopt, 2)))
	{
	  ...
	  for (enc_opt = opt6_ptr(opt, offset); enc_opt; enc_opt = opt6_user_vendor_next(enc_opt, enc_end))
	    if (opt6_user_vendor_len(enc_opt) >= vendor->len)
...
static void log6_opts(int nest, unsigned int xid, void *start_opts, void *end_opts)
{
  ...
  for (opt = start_opts; opt; opt = opt6_next(opt, end_opts))
      ...
      else if (type == OPTION6_STATUS_CODE)
	{
	  ...
	  int copy_len = opt6_len(opt) - 2;
	  if (copy_len > MAXDNAME - len - 1)
	    copy_len = MAXDNAME - len - 1;
	  if (copy_len > 0)
	    memcpy(daemon->namebuff + len, opt6_ptr(opt, 2), copy_len);
	  daemon->namebuff[len + (copy_len > 0 ? copy_len : 0)] = 0;
	  optname = "status";
	}
      else
	{

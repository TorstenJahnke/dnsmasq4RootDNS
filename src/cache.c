...
void cache_init(void)
{
  ...
  if (daemon->cachesize > 0)
    {
      if ((size_t)daemon->cachesize > SIZE_MAX / sizeof(struct crec))
	{
	  my_syslog(LOG_ERR, _("cache size too large, reducing"));
	  daemon->cachesize = (int)(SIZE_MAX / sizeof(struct crec));
	}
...
int cache_recv_insert(time_t now, int fd)
{
  ...
  switch (op)
	...
	if (uid == forward->uid)
	  {
	    forward->work_counter = keycount;
	    forward->validate_counter = validatecount;
  ...
  for (hr = daemon->host_records; hr; hr = hr->next)
	...
	if ((hr->flags & HR_4) &&
	    (cache = whine_malloc(SIZEOF_POINTER_CREC)))
	  {
	    ...
	    add_hosts_entry(cache, &a, INADDRSZ, SRC_CONFIG, (struct crec **)daemon->packet, revhashsz);
	  }
	...
	if ((hr->flags & HR_6) &&
	    (cache = whine_malloc(SIZEOF_POINTER_CREC)))
	  {
	    ...
	    add_hosts_entry(cache, &a, IN6ADDRSZ, SRC_CONFIG, (struct crec **)daemon->packet, revhashsz);
	  }
      }
...
static void dump_cache_entry(struct crec *cache, time_t now)
{
  ...
  *a = 0;
  ...
  {
    char timebuf[26];
    p += sprintf(p, "%-24.24s", cache->flags & F_IMMORTAL ? "" : ctime_r(&(cache->ttd), timebuf));
  }
#endif
...
void dump_cache(time_t now)
{
  ...
  blockdata_report();
  my_syslog(LOG_INFO, _("child processes for TCP requests: in use %u, highest since last SIGUSR1 %u, max allowed %d."),
  ...
  for (serv = daemon->servers; serv; serv = serv->next)
	...
	my_syslog(LOG_INFO, _("server %s#%d: queries sent %u, retried %u, failed %u, nxdomain replies %u, avg. latency %ums"),
		  daemon->addrbuff, port, queries, retrys, failed_queries, nxdomain_replies,
		  count_latency ? sigma_latency / count_latency : 0);
      }
...
void log_query(unsigned int flags, char *name, union all_addr *addr, char *arg, unsigned short type)
{
  ...
  if (addr)
	...
	{
	  snprintf(keytag_buf, sizeof(keytag_buf), arg,
	  ...
	  if (addr->log.ede != EDE_UNSET)
	    {
	      ...
	      snprintf(extra, ADDRSTRLEN, " (EDE: %s)", edestr(addr->log.ede));
	    }
	}

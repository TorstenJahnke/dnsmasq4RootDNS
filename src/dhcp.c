...
#  endif
  ...
  if ((is_relay_use_source || mess->giaddr.s_addr) && !is_relay_reply)
  ...
  else
    {
      ...
      *((struct sockaddr_in *)&req.arp_pa) = dest;
      ...
      if (ioctl(daemon->dhcpfd, SIOCSARP, &req) == -1)
	my_syslog(MS_DHCP | LOG_ERR, _("ARP-cache injection failed: %s"), strerror(errno));
    }
#elif defined(HAVE_BSD_NETWORK)
  else 
  ...
  if (setsockopt(fd, IPPROTO_IP, IP_BOUND_IF, &iface_index, sizeof(iface_index)) == -1)
    my_syslog(MS_DHCP | LOG_ERR, _("failed to set IP_BOUND_IF on DHCP socket: %s"), strerror(errno));
#endif

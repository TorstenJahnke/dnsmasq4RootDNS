...
static u16 read_u16(const void *p, int offset)
{
  u16 val;
  memcpy(&val, (const unsigned char *)p + offset * 2, sizeof(val));
  return val;
}
			   ...
			   union mysockaddr *src, union mysockaddr *dst, int port, int proto)
{
  ...
  if (family == AF_INET6)
	...
	{
	  sum += (ip6.ip6_src.s6_addr[i] << 8) + ip6.ip6_src.s6_addr[i+1];
	  sum += (ip6.ip6_dst.s6_addr[i] << 8) + ip6.ip6_dst.s6_addr[i+1];
	}
    }
  else
    {

...
static volatile DIR *mem_recover_dir = NULL;
...
#define LOPT_FWD_ROOTDNS   391
#define LOPT_ROOT_HINTS    392
#define LOPT_ROOT_PRIMING  393
#define LOPT_DELEG_CACHE_SIZE 394
#define LOPT_ROOT_ZONE     395
#define LOPT_WIPE_DNSSEC   396
#define LOPT_CNAME_FLAT    397
#define LOPT_CNAME_MINIM   398
#define LOPT_ITER_ASYNC    399
...
static const struct myoption opts[] = 
  ...
  { 
    ...
    { "forward-rootDNS", 0, 0, LOPT_FWD_ROOTDNS },
    { "with-root-hints", 1, 0, LOPT_ROOT_HINTS },
    { "with-root-priming", 0, 0, LOPT_ROOT_PRIMING },
    { "deleg-cache-size", 1, 0, LOPT_DELEG_CACHE_SIZE },
    { "with-root-zone", 1, 0, LOPT_ROOT_ZONE },
    { "wipe-dnssec", 0, 0, LOPT_WIPE_DNSSEC },
    { "cname-flattening", 0, 0, LOPT_CNAME_FLAT },
    { "cname-minimization", 0, 0, LOPT_CNAME_MINIM },
    { "iterative-async", 1, 0, LOPT_ITER_ASYNC },
    { NULL, 0, 0, 0 }
  };
...
static struct {
  ...
  { LOPT_FWD_ROOTDNS, OPT_RECURSIVE, NULL, gettext_noop("Resolve queries iteratively from DNS root servers (no forwarder needed)."), NULL },
  { LOPT_ROOT_HINTS, ARG_ONE, "<path>", gettext_noop("Load root server addresses from hints file (requires --forward-rootDNS)."), NULL },
  { LOPT_ROOT_PRIMING, OPT_ROOT_PRIMING, NULL, gettext_noop("Prime root server cache at startup by querying NS . (default with --forward-rootDNS)."), NULL },
  { LOPT_DELEG_CACHE_SIZE, ARG_ONE, "<entries>", gettext_noop("Set delegation cache size (default 4096). Used with --forward-rootDNS."), NULL },
  { LOPT_ROOT_ZONE, ARG_ONE, "<path>", gettext_noop("Pre-load all TLD delegations from root zone file (ultra-fast, no root queries needed)."), NULL },
  { LOPT_WIPE_DNSSEC, OPT_WIPE_DNSSEC, NULL, gettext_noop("Strip all DNSSEC records (DS, RRSIG, NSEC, DNSKEY) from DNS responses."), NULL },
  { LOPT_CNAME_FLAT, OPT_CNAME_FLAT, NULL, gettext_noop("Flatten CNAME chains: return only final A/AAAA records under the original query name."), NULL },
  { LOPT_CNAME_MINIM, OPT_CNAME_MINIM, NULL, gettext_noop("Minimize CNAME chains: return only first CNAME and final answer, strip intermediates."), NULL },
  { LOPT_ITER_ASYNC, ARG_ONE, "<N>", gettext_noop("Max concurrent async NS resolutions (0=sync, default 2). Prevents eventloop blocking."), NULL },
  { 0, 0, NULL, NULL, NULL }
};
...
static int parse_dhcp_opt(char *errstr, char *arg, int flags)
{
  ...
  if (comma)
      ...
      if (found_dig && (opt_len & OT_TIME) && strlen(comma) > 0)
	{
	  ...
	  {
	    unsigned long lval = atoi(comma);
	    u32 hval;
	    if (lval > 0x7fffffffUL / (unsigned long)fac)
	      lval = 0x7fffffff;
	    else
	      lval *= fac;
	    hval = htonl((u32)lval);
	    memcpy(new->val, &hval, 4);
	  }
	}  
...
{      
  ...
  switch (option)
    ...
    case LOPT_ROOT_HINTS:
      daemon->root_hints_file = opt_string_alloc(arg);
      break;

    case LOPT_ROOT_ZONE:
      daemon->root_zone_file = opt_string_alloc(arg);
      break;

    case LOPT_DELEG_CACHE_SIZE:
      {
	int size;
    ...
    case LOPT_ITER_ASYNC:
      {
	int limit;

	if (!atoi_check(arg, &limit))
	  ret_err(gen_err);
	...
	if (limit < 0)
	  ...
	  limit = 10;
	daemon->iterative_async_limit = limit;
      }
      break;
		...
		if ((netpart = split_chr(comma, '/')))
		  {
			...
			mask = msize == 0 ? (int)0xFFFFFFFF : (int)((1u << (32 - msize)) - 1);
			new->is6 = 0; 			  
		...
		/* dhcp-host has strange backwards-compat needs. */
		...
		else if (arg[0] == '[' && arg[strlen(arg)-1] == ']')
		  {
			...
			if (!atoi_check(pref, &new_addr->prefixlen) ||
			  ...
			  bad_prefix = 1;
			else if (new_addr->prefixlen <= 64)
			  {
			    if (addrpart != 0)
			      bad_prefix = 1;
			  }
			else if (new_addr->prefixlen < 128)
			  {
			    unsigned int hostbits = 128 - new_addr->prefixlen;
			    u64 hostmask = ((u64)1 << hostbits) - 1;
			    if (addrpart & hostmask)
			      bad_prefix = 1;
			  }
...
err:
    ...
    case LOPT_DUID: /* --dhcp-duid */
      {
	int duid_tmp;
	if (!(comma = split(arg)) || !atoi_check(arg, &duid_tmp))
	  ret_err(_("bad DUID"));
	else
	  {
	    daemon->duid_enterprise = (unsigned int)duid_tmp;
...
static void read_file(char *file, FILE *f, int hard_opt, int from_script)	
{
  ...
  while (fgets(buff, MAXDNAME, f))
    {
      volatile int white, i;
      volatile int option;
      char * volatile errmess, * volatile p, * volatile arg, * volatile start;
      size_t len;
	...
	{
	  if (from_script)
	    snprintf(daemon->namebuff + strlen(daemon->namebuff), MAXDNAME - strlen(daemon->namebuff), _(" in output from %s"), file);
	  else
	    snprintf(daemon->namebuff + strlen(daemon->namebuff), MAXDNAME - strlen(daemon->namebuff), _(" at line %d of %s"), lineno, file);
...
void read_opts(int argc, char **argv, char *compile_opts)
{
  ...
  daemon->iterative_async_limit = -1;
...
#define TESTLOOP 2      
  ...
  if (daemon->authserver && !daemon->hostmaster)
    {
      safe_strncpy(buff, "hostmaster.", MAXDNAME);
      strncat(buff, daemon->authserver, MAXDNAME - 1 - strlen(buff));
